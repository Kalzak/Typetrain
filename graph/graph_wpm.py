...
def plot_data():
    ...
    actual_wpm = [item['a_wpm'] for item in sentence_history]
    potential_wpm = [item['p_wpm'] for item in sentence_history]

    # 3. Use matplotlib to plot the graph
    plt.plot(sentence_numbers, actual_wpm, marker='o', color='b', label='Actual WPM')
    plt.plot(sentence_numbers, potential_wpm, marker='o', color='r', linestyle='--', label='Potential WPM')
